...
function performNlsExtract() {
    cp.spawnSync('yarn', [
        'theia', 'nls-extract',
        '-o', './oniro-ide-extension/i18n/nls.json',
        '-f', './oniro-ide-extension/src/browser/**/*.{ts,tsx}'
    ], {
        shell: true
    });
}
...
function hasNlsFileChanged() {
    const childProcess = cp.spawnSync('git', ['diff', '--exit-code', './oniro-ide-extension/i18n/nls.json']);
...
function performDeepLTranslation(token) {
    cp.spawnSync('yarn', [
        'theia', 'nls-localize',
        '-f', './oniro-ide-extension/i18n/nls.json',
        '--free-api', '-k', token,
